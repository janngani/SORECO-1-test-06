...
export const LandingPage = () => {
  ...
  >
    ...
    key={i}
    whileHover={{ y: -5 }}
    className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm text-center flex flex-col items-center min-h-[220px]"
  >
                {card.icon && <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center mb-6 shadow-inner">
                    {card.icon}
                  </div>}
                {card.title && <h4 className="text-lg font-bold text-slate-900 font-poppins mb-3">{card.title}</h4>}
                {card.desc && <p className="text-xs text-slate-500 leading-relaxed">{card.desc}</p>}
              </motion.div>)}
          </div>
        </div>
      </section>
  ...
  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {newsArticles.map((article, i) => <motion.div
    key={article.id}
    whileHover={{ y: -6 }}
